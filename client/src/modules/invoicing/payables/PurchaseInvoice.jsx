...
const PURCHASE_INVOICES = [
  { id: 'PINV-2026-00001', supplier: 'Shree Cement Ltd', items: 'Portland Cement x 500 Bags', amount: 190000, outstanding: 0, dueDate: '20/06/2026', status: 'Paid' },
  { id: 'PINV-2026-00002', supplier: 'Tata Steel', items: 'TMT Bars 12mm x 5000 KG', amount: 325000, outstanding: 162500, dueDate: '30/06/2026', status: 'Partly Paid' },
  { id: 'PINV-2026-00003', supplier: 'Kajaria Ceramics', items: 'Floor Tiles x 5000 SqFt', amount: 275000, outstanding: 275000, dueDate: '15/07/2026', status: 'Unpaid' },
  { id: 'PINV-2026-00004', supplier: 'Asian Paints', items: 'Exterior Paint x 500 Litres', amount: 90000, outstanding: 0, dueDate: '10/06/2026', status: 'Paid' },
  { id: 'PINV-2026-00005', supplier: 'Finolex Cables', items: 'Electrical Wire x 3000 Mtr', amount: 84000, outstanding: 84000, dueDate: '31/07/2026', status: 'Unpaid' },
  { id: 'PINV-2026-00006', supplier: 'Local Sand Supplier', items: 'River Sand x 1000 CFT', amount: 45000, outstanding: 0, dueDate: '05/06/2026', status: 'Paid' },
];

const SUPPLIERS = ['Shree Cement Ltd', 'Tata Steel', 'Kajaria Ceramics', 'Asian Paints', 'Finolex Cables', 'Local Sand Supplier'];

function Modal({ onClose }) {
  const [items, setItems] = useState([{ desc: '', qty: 1, rate: 0, tax: 18, amount: 0 }]);
  const addItem = () => setItems(p => [...p, { desc: '', qty: 1, rate: 0, tax: 18, amount: 0 }]);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#ffffff', borderRadius: '10px', width: '720px', maxHeight: '92vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid #e5e7eb' }}>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#1a1a2e' }}>New Purchase Invoice</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280' }}><X size={18} /></button>
        </div>
        <div style={{ padding: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            <div>
              <label style={{ fontSize: '12px', color: '#6b7280', display: 'block', marginBottom: '4px' }}>Supplier</label>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ minWidth: '240px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: '13px', color: '#6b7280' }}><span>Subtotal</span><span>₹{subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: '13px', color: '#6b7280' }}><span>Tax (18%)</span><span>₹{tax.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0 4px', fontSize: '14px', fontWeight: 700, color: '#1a1a2e', borderTop: '1px solid #e5e7eb', marginTop: '4px' }}><span>Total</span><span>₹{total.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
            </div>
          </div>
  ...
  return (
    <div style={{ padding: '20px 24px', backgroundColor: 'var(--bg-color, #f8f9fa)', minHeight: '100%', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 600, color: '#1a1a2e', margin: 0 }}>Purchase Invoice</h1>
      ...
      <div style={{ backgroundColor: 'var(--card-bg, #ffffff)', border: '1px solid var(--border-color, #e5e7eb)', borderRadius: '8px', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
              {['', 'ID', 'Supplier', 'Items', 'Amount', 'Outstanding', 'Due Date', 'Status'].map(h => (
            ...
            {PURCHASE_INVOICES.map((inv) => {
              ...
              return (
                <tr key={inv.id} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.1s' }}
                  ...
                  <td style={{ padding: '12px 14px', fontSize: '13px', fontWeight: 500, color: '#1a1a2e' }}>{inv.supplier}</td>
                  <td style={{ padding: '12px 14px', fontSize: '13px', color: '#374151' }}>{inv.items}</td>
