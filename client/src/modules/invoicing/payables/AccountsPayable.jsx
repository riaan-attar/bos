...
const STAT_CARDS = [
  { label: 'Total Payable', value: '₹6,06,500', color: '#1a1a2e' },
  { label: 'Overdue', value: '₹0', color: '#dc2626' },
  { label: 'Due This Month', value: '₹3,59,000', color: '#a16207' },
  { label: 'Paid', value: '₹3,25,000', color: '#15803d' },
];

const AGING_DATA = [
  { bucket: '0-30 days', amount: 359000, fill: '#22c55e' },
  { bucket: '31-60 days', amount: 247500, fill: '#f59e0b' },
  { bucket: '61-90 days', amount: 0, fill: '#ef4444' },
  { bucket: '90+ days', amount: 0, fill: '#7c3aed' },
];

const AP_DATA = [
  { supplier: 'Tata Steel', invoice: 'PINV-2026-00002', amount: 325000, outstanding: 162500, dueDate: '30/06/2026', age: 20, status: 'Partly Paid' },
  { supplier: 'Kajaria Ceramics', invoice: 'PINV-2026-00003', amount: 275000, outstanding: 275000, dueDate: '15/07/2026', age: 0, status: 'Unpaid' },
  { supplier: 'Finolex Cables', invoice: 'PINV-2026-00005', amount: 84000, outstanding: 84000, dueDate: '31/07/2026', age: 0, status: 'Unpaid' },
];
  ...
  return (
    <div style={{ padding: '20px 24px', backgroundColor: 'var(--bg-color, #f8f9fa)', minHeight: '100%', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 600, color: '#1a1a2e', margin: '0 0 4px 0' }}>Accounts Payable</h1>
        <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Outstanding supplier invoices report</p>
      </div>
      ...
      <div style={{ backgroundColor: 'var(--card-bg, #ffffff)', border: '1px solid var(--border-color, #e5e7eb)', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={AGING_DATA} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
            ...
            <YAxis tickFormatter={v => v >= 100000 ? `₹${(v / 100000).toFixed(0)}L` : `₹${(v / 1000).toFixed(0)}K`} tick={{ fontSize: 11, fill: '#6b7280' }} axisLine={false} tickLine={false} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
              {['Supplier', 'Invoice', 'Amount', 'Outstanding', 'Due Date', 'Age (days)', 'Status'].map(h => (
            ...
            {AP_DATA.map((row, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.1s' }}
                  ...
                  <td style={{ padding: '12px 14px', fontSize: '13px', fontWeight: 500, color: '#1a1a2e' }}>{row.supplier}</td>
