...
import { Plus, X, ChevronDown, RefreshCw, MoreHorizontal, Filter, FileText, List, ArrowUpDown, Columns, Layers } from 'lucide-react';
import { Link } from 'react-router-dom';

const MOCK_DATA = [
  { projectType: 'Residential', description: 'Housing and apartment projects' },
  { projectType: 'Commercial', description: 'Office buildings, malls, complexes' },
  { projectType: 'Industrial', description: 'Factories, warehouses, estates' },
  { projectType: 'Internal', description: 'Internal infrastructure projects' },
];
...
export default function ProjectType() {
  ...
  const filteredData = MOCK_DATA.filter(item => {
    return item.projectType.toLowerCase().includes(filterName.toLowerCase());
  });
  ...
  return (
    <div style={{ fontFamily: 'var(--font-family, Inter, sans-serif)', color: 'var(--text-color, #1a1a1a)', backgroundColor: 'var(--bg-color, #ffffff)', minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {showFilters && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', backgroundColor: 'var(--surface-gray-1, #f9fafb)', borderBottom: '1px solid var(--border-color, #e5e7eb)', padding: '12px 24px', alignItems: 'center' }}>
          <input placeholder="Project Type" value={filterName} onChange={e => setFilterName(e.target.value)} style={inputStyle} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-color, #ffffff)' }}>
        {filteredData.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', padding: '40px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted, #6b7280)', marginBottom: '16px', textAlign: 'center' }}>No Project Type found.</p>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--text-color, #1a1a1a)', fontWeight: 500 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '24px', height: '24px', backgroundColor: 'var(--surface-gray-1, #f9fafb)', border: '1px solid var(--border-color, #e5e7eb)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Layers size={12} color="var(--text-muted, #6b7280)" />
                        </div>
                        {row.projectType}
                      </div>
                    </td>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'var(--control-bg, #ffffff)', borderRadius: '10px', width: '400px', maxWidth: '90%', border: '1px solid var(--border-color, #e5e7eb)', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border-color, #e5e7eb)' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, margin: 0, color: 'var(--text-color, #1a1a1a)' }}>New Project Type</h2>
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div><label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text-muted, #4b5563)', display: 'block', marginBottom: '6px' }}>Project Type Name *</label><input type="text" style={inputStyle} /></div>
