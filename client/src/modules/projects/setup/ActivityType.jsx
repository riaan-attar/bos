...
import { Plus, X, ChevronDown, RefreshCw, MoreHorizontal, Filter, FileText, List, ArrowUpDown, Columns, CheckSquare } from 'lucide-react';
import { Link } from 'react-router-dom';

const MOCK_DATA = [
  { activityType: 'Site Inspection', billingType: 'Hourly', billingRate: '₹2,000/hr', costingRate: '₹1,500/hr' },
  { activityType: 'Construction Work', billingType: 'Daily', billingRate: '₹5,000/day', costingRate: '₹4,000/day' },
  { activityType: 'Design & Planning', billingType: 'Fixed Rate', billingRate: '₹50,000', costingRate: '₹40,000' },
  { activityType: 'Client Meeting', billingType: 'Hourly', billingRate: '₹1,500/hr', costingRate: '₹1,000/hr' },
];
...
export default function ActivityType() {
  ...
  const filteredData = MOCK_DATA.filter(item => {
    return item.activityType.toLowerCase().includes(filterName.toLowerCase()) &&
           (filterBillingType ? item.billingType === filterBillingType : true);
  });

  const clearFilters = () => { setFilterName(''); setFilterBillingType(''); };
    ...
    <div style={{ fontFamily: 'var(--font-family, Inter, sans-serif)', color: 'var(--text-color, #1a1a1a)', backgroundColor: 'var(--bg-color, #ffffff)', minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 24px', borderBottom: '1px solid var(--border-color, #e5e7eb)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={() => setIsModalOpen(true)} style={{ backgroundColor: 'var(--gray-900, #111827)', border: '1px solid var(--gray-900, #111827)', color: '#ffffff', padding: '6px 14px', borderRadius: '6px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '8px' }}>
            <Plus size={14} /> Add Activity Type
          </button>
        </div>
      </div>
      ...
      {showFilters && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', backgroundColor: 'var(--surface-gray-1, #f9fafb)', borderBottom: '1px solid var(--border-color, #e5e7eb)', padding: '12px 24px', alignItems: 'center' }}>
          <input placeholder="Activity Name" value={filterName} onChange={e => setFilterName(e.target.value)} style={inputStyle} />
          <select value={filterBillingType} onChange={e => setFilterBillingType(e.target.value)} style={inputStyle}>
            <option value="">Billing Type (All)</option><option value="Hourly">Hourly</option><option value="Daily">Daily</option><option value="Fixed Rate">Fixed Rate</option>
          </select>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-color, #ffffff)' }}>
        {filteredData.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', padding: '40px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted, #6b7280)', marginBottom: '16px', textAlign: 'center' }}>No Activity Type found.</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-color, #e5e7eb)', color: 'var(--text-muted, #6b7280)' }}>
                  ...
                  <th style={{ padding: '12px 16px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>BILLING TYPE</th>
              ...
              <tbody>
                {filteredData.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border-color, #f3f4f6)', cursor: 'pointer', transition: 'background-color 0.15s' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--surface-gray-1, #f9fafb)'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--text-muted, #4b5563)' }}>{row.billingType}</td>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'var(--control-bg, #ffffff)', borderRadius: '10px', width: '400px', maxWidth: '90%', border: '1px solid var(--border-color, #e5e7eb)', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border-color, #e5e7eb)' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, margin: 0, color: 'var(--text-color, #1a1a1a)' }}>New Activity Type</h2>
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div><label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text-muted, #4b5563)', display: 'block', marginBottom: '6px' }}>Activity Type Name *</label><input type="text" style={inputStyle} /></div>
              <div><label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text-muted, #4b5563)', display: 'block', marginBottom: '6px' }}>Billing Type</label><select style={inputStyle}><option>Hourly</option><option>Daily</option><option>Fixed Rate</option></select></div>
            </div>
