/**
 * LogCallModal — client/src/modules/crm/customers/components/LogCallModal.jsx
 * Modal for logging a phone call to the customer's activity timeline.
...
export default function LogCallModal({ isOpen, onClose, onSave }) {
  ...
  useEffect(() => {
    if (isOpen) { setOutcome('Answered'); setDuration(''); setNotes(''); }
  }, [isOpen]);
    ...
    >
      <div
        style={{
          ...
          borderRadius: '10px', width: '440px', maxWidth: '94vw',
          padding: '24px', position: 'relative',
        }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
          <span style={{ fontSize: '16px', fontWeight: '600', color: '#f8f8f8' }}>Log a Call</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            ...
            <FocusInput as="select" value={outcome} onChange={e => setOutcome(e.target.value)} style={{ cursor: 'pointer' }}>
              {OUTCOME_OPTIONS.map(opt => <option key={opt} value={opt} style={{ background: '#232323' }}>{opt}</option>)}
            </FocusInput>
          </div>
          <div>
            <label style={labelStyle}>Duration</label>
            <FocusInput type="text" value={duration} onChange={e => setDuration(e.target.value)} placeholder="e.g. 5 mins" />
          </div>
          <div>
            <label style={labelStyle}>Notes</label>
            <FocusInput as="textarea" rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="What was discussed..." style={{ resize: 'vertical' }} />
          </div>
        </div>
        ...
        <div style={{ marginTop: '22px', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
          ...
          >Cancel</button>
          <button onClick={() => onSave({ outcome, duration, notes })}
